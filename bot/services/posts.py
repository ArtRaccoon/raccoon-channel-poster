...
async def get_post(db_path: str, post_id: int):
    async with aiosqlite.connect(db_path) as db:
        cur = await db.execute('SELECT id, owner_telegram_id, channel_id, text, media_file_id, media_type, status, created_at, published_at, scheduled_at FROM posts WHERE id=?', (post_id,))
...
async def get_due_scheduled_posts(db_path: str, now_iso: str):
    async with aiosqlite.connect(db_path) as db:
        cur = await db.execute(
            "SELECT id, owner_telegram_id, channel_id, text, media_file_id, media_type FROM posts WHERE status='scheduled' AND scheduled_at IS NOT NULL AND datetime(scheduled_at) <= datetime(?) ORDER BY id ASC LIMIT 50",
            (now_iso,),
        )
        return await cur.fetchall()
