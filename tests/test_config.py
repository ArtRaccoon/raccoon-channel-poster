import pytest
from bot.config import Config, ConfigError, parse_admin_ids
from bot.proxy import safe_proxy_url

def test_parse_admin_ids():
    assert parse_admin_ids('1, 2,3') == {1,2,3}

def test_env_loads_without_channel(monkeypatch):
...
def test_proxy_enabled_disabled(monkeypatch):
    monkeypatch.setenv('BOT_TOKEN','x'); monkeypatch.setenv('ADMIN_IDS','1')
